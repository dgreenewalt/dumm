...
var map = new ol.Map({
    ...
    view: new ol.View({
         maxZoom: 28, minZoom: 5, projection: new ol.proj.Projection({
            code: 'EPSG:32145',
            extent: [418434.100000, 20827.800000, 583953.500000, 285253.350000],
            units: 'm'})
    })
});
...
map.getView().fit([481744.639141, 41358.366788, 501912.005379, 54056.843837], map.getSize());
			...
			if (doPopup) {
                ...
                for (var i=0; i<currentFeatureKeys.length; i++) {
                    if (currentFeatureKeys[i] != 'geometry') {
                        ...
                        txt = (currentFeature.get(currentFeatureKeys[i]) != null ? autolinker.link(currentFeature.get(currentFeatureKeys[i]).toLocaleString()) : '');
                        //if (txt != ''){popupField += '<td>';}
                        
						if (layer.get('fieldAliases')[currentFeatureKeys[i]] == 'PARCNUM'){popupField += '<td>Parcel Number</td><td>' + txt + '</td>'}
						if (layer.get('fieldAliases')[currentFeatureKeys[i]] == 'e911'){popupField += '<td>Location</td><td>' + txt + '</td>'}
						if (layer.get('fieldAliases')[currentFeatureKeys[i]] == 'P_REAL'){popupField += '<td>Real Value</td><td>$'  + txt + '</td>'}
						if (layer.get('fieldAliases')[currentFeatureKeys[i]] == 'P_BOOK'){popupField += '<td>Book</td><td>' + txt + '</td>'}
						if (layer.get('fieldAliases')[currentFeatureKeys[i]] == 'P_PAGE'){popupField += '<td>Page</td><td>' + txt + '</td>'}
						if (layer.get('fieldAliases')[currentFeatureKeys[i]] == 'P_TOTL'){popupField += '<td>Acres Total</td><td>' + txt + ' Ac</td>'}
						if (layer.get('fieldAliases')[currentFeatureKeys[i]] == 'P_SALEDATE'){popupField += '<td>Sale Date</td><td>' + txt + '</td>'}
						if (layer.get('fieldAliases')[currentFeatureKeys[i]] == 'P_LUACRES'){
							if (txt != '0'){popupField += '<td>Current Use</td><td>' + txt + ' Ac</td>'}
						}
						if (layer.get('fieldAliases')[currentFeatureKeys[i]] == 'P_NAME1'){
							popupField += '<td>Owner</td><td>' + txt + '<br>'
							i++
							txt = (currentFeature.get(currentFeatureKeys[i]) != null ? autolinker.link(currentFeature.get(currentFeatureKeys[i]).toLocaleString()) : '');
							if (txt != ''){popupField +=  txt + '<br>'} //name2
							i++
							txt = (currentFeature.get(currentFeatureKeys[i]) != null ? autolinker.link(currentFeature.get(currentFeatureKeys[i]).toLocaleString()) : '');
							popupField +=  txt + '<br>'  //address A
							i++
							txt = (currentFeature.get(currentFeatureKeys[i]) != null ? autolinker.link(currentFeature.get(currentFeatureKeys[i]).toLocaleString()) : '');
							if (txt != ''){popupField +=  txt + '<br>'}//address b
							i++
							txt = (currentFeature.get(currentFeatureKeys[i]) != null ? autolinker.link(currentFeature.get(currentFeatureKeys[i]).toLocaleString()) : '');
							popupField +=  txt + '</td>'  //city state zip
						}

                        if (popupField != ''){popupText += '<tr>' + popupField + '</tr>'}
                    }
                }
                popupText += '</table></li>';
            }
